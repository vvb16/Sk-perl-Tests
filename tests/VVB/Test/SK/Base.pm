...
sub SK_command_execute
{
  ...
  $result = sendMessage($VVB::Test::Base::channel, $timeout,
    $self->{device_id}, length($data) + 2,
    pack('C2', $self->{cmd_count}, $command) . $data);
  ...
  return dualvar(-1, sprintf("answer command counter should be %d got %d", $self->{cmd_count}, $cmd_count)) if $cmd_count != $self->{cmd_count};
...
sub find_devices #: Tests(3)
{
  ...
  while (1)
  {
    ...
    push $self->{devices}, {id => $id, serial => $serial};
  }
  return dualvar(0, "");
}
...
sub startup : Tests(startup => 3)
{
  ...
  SKIP: {
    skip "Already initialized", 3 if $self->{devices};
    $self->{devices} = [];
    $self->SUPER::startup();
    $self->check_CAN_channel();
    my $result = $self->find_devices();
    cmp_ok($result, '==', 0, "find_devices should succeed") || diag $result;
    cmp_ok(scalar(@{$self->{devices}}), '>', 0, "at least one device should present");
  }
}
...
sub t05_SW_VER : Tests(2)
{
  ...
  my $result = $self->SK_command_execute(100, 0x00, '');
...
sub t06_runtime_stats : Tests(21)
{
  ...
  for my $descr (@dict)
  {
    my $result = $self->SK_command_execute(100, 0x01, pack('C', $code));
