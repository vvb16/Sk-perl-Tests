...
sub _t01_Pulser_found : Tests(1)
{
  ...
  my @pulsers = grep { $_->{id} == 0x10 } @{$test->{devices}};
  is(scalar @pulsers, 1, "Exactly one Pulser should be found");
  $test->set_device_id(0x10); 
  $test->reset_cmd_count(); 
}
...
sub t10_device_default_mode : Tests(3)
{
  ...
  my $result = $test->SK_command_execute(100, 0x20, pack('C2', 0, 0));
...
sub t11_device_status : Tests(4)
{
  ...
  my $code = 1;
    ...
    my $result = $test->SK_command_execute(100, 0x06, pack('C1', $code));
    cmp_ok($result, '==', 0, "command 0x06 should succeed") || diag $result;
...
sub t20_clear_mode : Tests(12)
{
  ...
  for my $mode (0..1)
  {
    my $result = $test->SK_command_execute(100, 0x23, pack('C2x1', 1, $mode));
    ...
    my $result = $test->SK_command_execute(100, 0x25, pack('C2', $mode, 0));
